...
def compute_a(z):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    t = np.exp(np.matrix(z, dtype=np.float128))
    a = t / np.sum(t)
...
def compute_L(a,y):
    ...
    ## INSERT YOUR#  CODE HERE
    temp_a = np.copy(a)
    temp_a[abs(temp_a) < 1e-7] = 1
    one_hot = np.zeros((np.shape(a)[0], 1))
    one_hot[y][0] = 1
    L = float(-1 * np.sum(np.multiply(one_hot, np.log(temp_a))))
    if( abs(a[y][0]) < 1e-5 ):
        L = 1e6
    #########################################
    return L 
...
def compute_da_dz(a):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    #da_dz = np.matrix(np.zeros((a.shape[0],a.shape[0])))
    da_dz = -1 * a * a.T
    row, col = np.diag_indices(a.shape[0])
    da_dz[row, col]= np.squeeze([item[0] * (1 -item[0]) for item in a])
    #########################################
    return da_dz 
...
def compute_dz_dW(x,c):
    ...
    x = np.squeeze(x)
    #print("squeezed x: ", x)
    dz_dW = np.zeros((c, 1), dtype=x.dtype) + x
    #########################################
    return dz_dW
...
def train(X, Y, alpha=0.01, n_epoch=100):
    ...
    for _ in range(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            W = update_W(W, dL_dW, alpha=0.01)
            b = update_b(b, dL_db, alpha=0.01)
            #########################################
    return W, b
